...
$db->query($query_rs);
$db->bind(1, htmlentities($rs_data['flight_sched_id']));
$rs_data_inspect = $db->rowsingle();
$rs_data_inspect_total=$db->rowcount();

$query_rs = "SELECT * FROM inspection_sched WHERE `flight_sched_id` = ? and inspection_type='POST-FLIGHT'";
$db->query($query_rs);
$db->bind(1, htmlentities($rs_data['flight_sched_id']));
$rs_data_inspect_post = $db->rowsingle();
$rs_data_inspect_post_total=$db->rowcount();
?>

<?php if ($rs_data_inspect_total>0) { if(file_exists('../images/aircraft_maintenance_sched/'.htmlentities($rs_data_inspect['inspection_sched_id']).'.pdf')) {?>
  <label class="col-sm-12 col-form-label"><strong>Pre-Inspection Report</strong></label>
  <div align="center">
    <object data="<?php echo '../images/aircraft_maintenance_sched/'.htmlentities($rs_data_inspect['inspection_sched_id']).'.pdf';?>"  width="100%"  height="700px"> </object>
  </div>  
<?php } } ?>

<br>
<?php  if(file_exists('../images/flight_sched/'.htmlentities($rs_data['flight_sched_id']).'.pdf')) {?>
  <label class="col-sm-12 col-form-label"><strong>Flight Plan Report</strong></label>
  <div align="center">
    <object data="<?php echo '../images/flight_sched/'.htmlentities($rs_data['flight_sched_id']).'.pdf';?>"  width="100%"  height="700px"> </object>
  </div>  
<?php }  ?>

<?php  if ($rs_data_inspect_post_total>0) { if(file_exists('../images/aircraft_maintenance_sched_post/'.htmlentities($rs_data_inspect_post['inspection_sched_id']).'.pdf')) {?>
  <label class="col-sm-12 col-form-label"><strong>Post-Inspection Report</strong></label>
  <div align="center">
    <object data="<?php echo '../images/aircraft_maintenance_sched_post/'.htmlentities($rs_data_inspect_post['inspection_sched_id']).'.pdf';?>"  width="100%"  height="700px"> </object>
  </div>  
<?php } } ?>
