...
if ((isset($_POST["POSTcheck"])) && ($_POST["POSTcheck"] == "form1")) {
      ...
      $SQLcrud = "UPDATE `lup_level` SET `level`=? WHERE `level`=?";
...
<form method="POST" id="form1" name="form1" onsubmit="return validateForm();">
    ...
    <fieldset>    
        ...
        <div class="form-group">
                ...
                <label >Level</label>
                <input required class="form-control" type="text" name="level" id="level"
              value="<?php if (isset($_POST['level'])) echo htmlentities($_POST['level']); else echo htmlentities($rs_data['level']); ?>" size="32" 
              OnKeyUp="showAjax('level_duplicate.php','txtString',this.value + '&prev_id=<?php echo htmlentities($rs_data['level']);?>' , 'results_here');"  placeholder=" ">
        ...
        <div class="form-group">
          ...
          <a href="level_list.php" class="btn btn-outline-danger"><span class="bi-x-octagon"></span> Cancel</a>
          </div>
        </div>
  ...
  <input type="hidden" name="id" value="<?php echo htmlentities($rs_data['level']); ?>">
